...
def create_site_figure(
    ...
    """
    Create the main site visualization with plume heatmap, paths, and assets.
    ...
    # Equipment pads first (under heatmap layer in visual order,
    # but Plotly draws traces in order so we add pads, then heatmap on top).
    # We render facility AFTER heatmap so lines are visible above the colour field.
